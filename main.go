...
func main() {
	...
	r.Handle("/menu", commonChain.Then(env.AppMiddleware(env.VMenuHandler))).Methods("GET")
	r.Handle("/search", commonChain.Then(env.AppMiddleware(env.VSearchHandler))).Methods("GET")
	...
	r.Handle("/{item:delete-token}", commonChain.Then(env.AppMiddleware(env.DeleteTokenHandler))).Methods("GET")
	...
	r.Handle("/{item:borrowings}", securechain.Then(env.AppMiddleware(env.ToogleStorageBorrowingHandler))).Methods("PUT")
