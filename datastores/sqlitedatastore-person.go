...
// HasPersonReadRestrictedProductPermission returns true if the person with id "id"
...
func (db *SQLiteDataStore) HasPersonReadRestrictedProductPermission(id int) (bool, error) {
	...
	(permission.permission_perm_name = "all" AND
	permission.permission_item_name = "all" AND
	permission_entity_id = -1) OR
	permission.permission_item_name = "rproducts"`
	if err = db.Get(&count, sqlr, id); err != nil {
		return false, err
	}
	globals.Log.WithFields(logrus.Fields{"id": id, "count": count}).Debug("HasPersonReadRestrictedProductPermission")
	if count == 0 {
		res = false
	} else {
		res = true
